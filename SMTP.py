...
def banner():
    ...
    print('       {1}-=[     {0}SMTP Cracker{1}                ]=-'.format(GREEN, WARNING))
...
try:
    ...
    thret=sys.argv[2]
    if int(thret)>200: #MAX AJA 200 !
        thret=200
        option=sys.argv[3]
except:
    banner()
    inputs=open(input('SMTP List : '),'r').read().splitlines()
    email=input("Your Email : ")#dont change this
    thret= 20
    if int(thret)>200:
        thret=200
